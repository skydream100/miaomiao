...
export default {
    ...
    watch : {
        message(newVal){
            // 防抖 --> 只要不是最后一次触发，就不做异步操作
            var that = this; // 也可以将 function (c) 这一块改成箭头函数

            // 切换城市的cityId
            var cityId = this.$store.state.city.id;

            // 调用 cancelRequest 这个方法 --> 取消上一次请求
            this.cancelRequest();
            this.axios.get('/api/searchList?cityId='+ cityId +'&kw='+newVal,{
                // 配置一个终止请求的方法。如果在多次请求时，就会在请求结束之前触发这个cancelToken
                // 然后将 c 赋值给source c --> function
